...
//               variant="ghost"
//               size="icon"
//               className="text-red-500 h-7 w-7 hover:text-red-400 hover:bg-red-900/20"
...
export function AttributesEditor({
  ...
  const handleAttributeChange = (name: string, value: string) => {
    ...
    // Update the component's trait value
    if (selectedElement) {
    ...
    // Call the parent's onAttributeChange
    onAttributeChange(name, value);
  };
  ...
  const handleTraitChange = (traitName: string, value: string) => {
    if (selectedElement) {
      ...
      if (trait) {
        // Set the trait value - this will automatically update the component
        trait.setValue(value);
  ...
  // Get element-specific attributes based on tag name
  ...
  return (
    <div className="space-y-6">
      {/* Display traits first if available */}
      {traits.length > 0 && (
              ...
              <div key={traitName} className="space-y-2">
                ...
                {traitType === "select" ? (
                  <select
                    value={traitValue || ""}
                    onChange={(e) =>
                      handleTraitChange(traitName, e.target.value)
                    }
                    className="w-full px-3 py-2 text-sm border rounded-md bg-slate-800 border-slate-700 focus:outline-none focus:ring-2 focus:ring-primary"
                  >
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      className="w-4 h-4 rounded text-primary bg-slate-800 border-slate-700 focus:ring-primary"
                    />
                  </div>
                ) : (
                  <Input
      ...
      {elementSpecificAttributes.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-sm font-semibold text-slate-200">
            {tagName?.toUpperCase()} Attributes
          </h3>
